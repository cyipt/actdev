...
We use commute data which disaggregates the points of origin and destination at the Medium Super Output Area (MSOA) level.
...
However, the census data is best seen as an indication of travel patterns in the local area surrounding a site, rather than a reflection of the site itself.
...
Having obtained data on commuter destinations and modes of travel, the next step is the identify the routes on the local road network that we expect these journeys to follow.
For cycling, we use a set of algorithms created by CycleStreets.net(link).
...
For the quiet routes, a 'quietness' parameter is maximised. This includes factors such as road type and characteristics. 
It also takes into account the gradient of route segments.
